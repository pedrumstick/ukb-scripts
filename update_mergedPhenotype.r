...
if (!file.exists(new_file)) {stop("The specified file to merge in does not exist.")}
...
for (input_file in list.files(pattern=paste0("UKB_MERGED_PHENO_.*_",old_date,"_FINAL_INCLUDE_PCs.txt.*"))) {
    ...
    updated_df <- fread(new_filePath, header=TRUE, stringsAsFactors=FALSE)
    message("There are ", ncol(updated_df)-1, " new columns of data in this update.")
    original_df <- fread(input_file, header=TRUE, stringsAsFactors=FALSE)
    exclude_df <- fread(withdrawn_consent_file, header=FALSE, stringsAsFactors=FALSE)
