...
if (!file.exists(input_file_diag)) {stop(paste0("ERROR: There is no specified input file ", input_file_diag))}
...
if (!file.exists(input_file_oper)) {stop(paste0("ERROR: There is no specified input file ", input_file_oper))}
output_dir <- "/hightide/UKB_PHENOS/Data_Portal_Downloads/Inpatient_Hospital/"
if (!dir.exists(output_dir)) {stop(paste0("ERROR: There is no specified output directory ", output_dir))}
...
icd9_diagnoses_clean <- setDT(as.data.frame(t(apply(icd9_diagnoses,1,function(x){c(x[!is.na(x)],x[is.na(x)])}))))
icd9_dates_clean <- setDT(as.data.frame(t(apply(icd9_dates,1,function(x){c(x[!is.na(x)],x[is.na(x)])}))))
...
# Check
if (!identical(dim(icd9_diagnoses_clean), dim(icd9_dates_clean))) {stop("ERROR: Dimensions of date and diagnosis codes are different!")}
# Output
...
# Shift non-NA cells to the left of the dataframe
# https://stackoverflow.com/questions/49079789/using-r-to-shift-values-to-the-left-of-data-frame
opcs3_diagnoses_clean <- setDT(as.data.frame(t(apply(opcs3_diagnoses,1,function(x){c(x[!is.na(x)],x[is.na(x)])}))))
opcs3_dates_clean <- setDT(as.data.frame(t(apply(opcs3_dates,1,function(x){c(x[!is.na(x)],x[is.na(x)])}))))
# Delete columns with all NA values
...
# Check
if (!identical(dim(opcs3_diagnoses_clean), dim(opcs3_dates_clean))) {stop("ERROR: Dimensions of date and diagnosis codes are different!")}
# Output
...
lapply(opcs4_diagnoses, function(x) {write.table(x, "temp_opcs4_diag.txt", sep="\t", row.names=FALSE, quote=FALSE, col.names=FALSE, append=TRUE)})
lapply(opcs4_dates, function(x) {write.table(x, "temp_opcs4_dates.txt", sep="\t", row.names=FALSE, quote=FALSE, col.names=FALSE, append=TRUE)})
...
lapply(icd10_diagnoses, function(x) {write.table(x, "temp_icd10_diag.txt", sep="\t", row.names=FALSE, quote=FALSE, col.names=FALSE, append=TRUE)})
lapply(icd10_dates, function(x) {write.table(x, "temp_icd10_dates.txt", sep="\t", row.names=FALSE, quote=FALSE, col.names=FALSE, append=TRUE)})
# Read data back in to assign column names, and fill in any columns with "NA"
